...
public class IntakeStateMachine extends StateMachine<IntakeStateMachine.IntakeState> {
    ...
    public static IntakeStateMachine getInstance() {
        if (instance == null) instance = new IntakeStateMachine();
        return instance;
    }
    ...
    protected void generateTransformations() {
        addTransformation(IntakeState.ENABLED, new InstantCommand(() -> intake.setEnabled(true)));
        addTransformation(IntakeState.DISABLED, new InstantCommand(() -> intake.setEnabled(false)));
    }
}
